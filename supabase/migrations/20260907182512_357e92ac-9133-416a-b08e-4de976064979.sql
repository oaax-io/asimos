...
BEGIN
  ...
  LOOP
    ...
    IF COALESCE(cardinality(p.preferred_property_types), 0) > 0
       AND NOT (NEW.property_type = ANY(p.preferred_property_types)) THEN CONTINUE; END IF;
    IF COALESCE(cardinality(p.preferred_cities), 0) > 0
       AND NOT EXISTS (
         SELECT 1
         FROM unnest(p.preferred_cities) AS ct
    ...
    FOR sub IN
      SELECT user_id
      FROM public.search_profile_subscriptions
