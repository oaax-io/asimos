...
import { Plus, Search, FileCheck2 } from "lucide-react";
...
const STATUS_LABELS = {
  draft: "Entwurf", pending: "Offen", signed: "Unterzeichnet", cancelled: "Storniert", expired: "Abgelaufen",
} as const;
const STATUS_VARIANTS: Record<string, "default" | "secondary" | "outline" | "destructive"> = {
  draft: "outline", pending: "default", signed: "secondary", cancelled: "destructive", expired: "secondary",
};
...
function ReservationsPage() {
  ...
  const [form, setForm] = useState({
    client_id: "", property_id: "", reservation_fee: "", valid_until: "", notes: "",
  });
  ...
  const updateStatus = useMutation({
    mutationFn: async ({ id, status }: { id: string; status: string }) => {
      const { error } = await supabase.from("reservations").update({ status: status as "draft" }).eq("id", id);
      if (error) throw error;
    },
  ...
  const filtered = useMemo(() => reservations.filter((r) => {
    ...
    if (search) {
      const c = (r.clients as { full_name: string } | null)?.full_name?.toLowerCase() ?? "";
      const p = (r.properties as { title: string } | null)?.title?.toLowerCase() ?? "";
  ...
  return (
    <>
      <PageHeader
        title="Reservationen"
        description="Reservationsverträge zwischen Käufern und Objekten"
        action={
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild><Button><Plus className="mr-1 h-4 w-4" />Neue Reservation</Button></DialogTrigger>
            <DialogContent>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div><Label>Reservationsgebühr (CHF)</Label><Input type="number" value={form.reservation_fee} onChange={(e) => setForm({ ...form, reservation_fee: e.target.value })} /></div>
      ...
      {isLoading ? (
        ...
        <EmptyState title="Keine Reservationen" description="Erstelle eine Reservation, sobald ein Käufer ein Objekt verbindlich sichern möchte." />
      ) : (
            ...
            <TableBody>
              {filtered.map((r) => (
                <TableRow key={r.id}>
                  ...
                  <TableCell>{(r.properties as { title: string } | null)?.title ?? "—"}</TableCell>
                  <TableCell>{formatCurrency(r.reservation_fee ? Number(r.reservation_fee) : null)}</TableCell>
                  <TableCell className="text-muted-foreground">{r.valid_until ? formatDate(r.valid_until) : "—"}</TableCell>
                  <TableCell>
                    <Select value={r.status} onValueChange={(v) => updateStatus.mutate({ id: r.id, status: v })}>
                      <SelectTrigger className="h-8 w-36">
                        <Badge variant={STATUS_VARIANTS[r.status]}>{STATUS_LABELS[r.status as keyof typeof STATUS_LABELS]}</Badge>
                      </SelectTrigger>
                      <SelectContent>
                        {Object.entries(STATUS_LABELS).map(([k, v]) => <SelectItem key={k} value={k}>{v}</SelectItem>)}
                      </SelectContent>
