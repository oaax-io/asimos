...
const STATUS_LABELS = {
  draft: "Entwurf", active: "Aktiv", expired: "Abgelaufen", cancelled: "Storniert", completed: "Abgeschlossen",
} as const;
const STATUS_VARIANTS: Record<string, "default" | "secondary" | "outline" | "destructive"> = {
  draft: "outline", active: "default", expired: "secondary", cancelled: "destructive", completed: "secondary",
};
...
function MandatesPage() {
  ...
  const [form, setForm] = useState({
    client_id: "", property_id: "", commission_model: "percent",
    commission_value: "", valid_from: "", valid_until: "", notes: "",
  });
  ...
  const create = useMutation({
    ...
    onSuccess: () => {
      ...
      setForm({ client_id: "", property_id: "", commission_model: "percent", commission_value: "", valid_from: "", valid_until: "", notes: "" });
      setOpen(false);
    },
  ...
  const updateStatus = useMutation({
    mutationFn: async ({ id, status }: { id: string; status: string }) => {
      const { error } = await supabase.from("mandates").update({ status: status as "draft" }).eq("id", id);
      if (error) throw error;
    },
  ...
  const filtered = useMemo(() => mandates.filter((m) => {
    ...
    if (search) {
      const c = (m.clients as { full_name: string } | null)?.full_name?.toLowerCase() ?? "";
      const p = (m.properties as { title: string } | null)?.title?.toLowerCase() ?? "";
    ...
    <>
      <PageHeader
        ...
        action={
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild><Button><Plus className="mr-1 h-4 w-4" />Neues Mandat</Button></DialogTrigger>
            <DialogContent>
              <DialogHeader><DialogTitle>Neues Mandat</DialogTitle></DialogHeader>
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div><Label>Wert</Label><Input type="number" step="0.01" value={form.commission_value} onChange={(e) => setForm({ ...form, commission_value: e.target.value })} /></div>
                </div>
                <div className="grid grid-cols-2 gap-3">
      ...
      <div className="mb-4 flex flex-wrap gap-3">
        <div className="relative max-w-sm flex-1">
          ...
          <Input className="pl-9" placeholder="Mandate suchen…" value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
      ...
      {isLoading ? (
        ...
        <EmptyState title="Keine Mandate" description="Lege dein erstes Mandat an, um Provisionen und Vertragsdetails zu verwalten." />
      ) : (
                ...
                <TableRow key={m.id}>
                  <TableCell className="font-medium">
                    <div className="flex items-center gap-2">
                      ...
                      {(m.clients as { full_name: string } | null)?.full_name ?? "—"}
                    </div>
                  </TableCell>
                  <TableCell>{(m.properties as { title: string } | null)?.title ?? "—"}</TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <Select value={m.status} onValueChange={(v) => updateStatus.mutate({ id: m.id, status: v })}>
                      <SelectTrigger className="h-8 w-36">
                        <Badge variant={STATUS_VARIANTS[m.status]}>{STATUS_LABELS[m.status as keyof typeof STATUS_LABELS]}</Badge>
                      </SelectTrigger>
                      <SelectContent>
                        {Object.entries(STATUS_LABELS).map(([k, v]) => <SelectItem key={k} value={k}>{v}</SelectItem>)}
                      </SelectContent>
