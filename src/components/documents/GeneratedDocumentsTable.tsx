...
export function GeneratedDocumentsTable({
  ...
  const { data: rows = [], isLoading } = useQuery({
    queryKey: ["generated-documents", filterRelatedType, filterRelatedId],
    queryFn: async () => {
      let q = supabase
      ...
      if (filterRelatedType) q = q.eq("related_type", filterRelatedType);
      if (filterRelatedId) q = q.eq("related_id", filterRelatedId);
  ...
  const filtered = useMemo(() => {
    return rows.filter((r) => {
      ...
      if (signFilter === "signed" && r.status !== "signed") return false;
      if (signFilter === "unsigned" && r.status === "signed") return false;
      if (sendFilter === "sent" && !r.sent_at) return false;
      if (sendFilter === "unsent" && r.sent_at) return false;
      if (search) {
  ...
  }, [rows, search, typeFilter, statusFilter, signFilter, sendFilter]);
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Select value={signFilter} onValueChange={setSignFilter}>
          <SelectTrigger className="w-[160px]"><SelectValue /></SelectTrigger>
          <SelectContent>
            <SelectItem value="all">Signatur: alle</SelectItem>
            <SelectItem value="signed">Unterzeichnet</SelectItem>
            <SelectItem value="unsigned">Nicht unterzeichnet</SelectItem>
          </SelectContent>
        </Select>
        <Select value={sendFilter} onValueChange={setSendFilter}>
          <SelectTrigger className="w-[160px]"><SelectValue /></SelectTrigger>
          <SelectContent>
            <SelectItem value="all">Versand: alle</SelectItem>
            <SelectItem value="sent">Versendet</SelectItem>
            <SelectItem value="unsent">Nicht versendet</SelectItem>
          </SelectContent>
        </Select>
      </div>
      ...
      {isLoading ? (
        ...
        <EmptyState
          title="Keine generierten Dokumente"
